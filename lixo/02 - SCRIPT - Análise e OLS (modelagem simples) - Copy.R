...
dados_f_pedidos <- dados_f_pedidos[, c("id_pedido", "id_cliente", "id_cidade", "Total_Pedido")]
dados_f_pedidos <- distinct(dados_f_pedidos)
...
fato_pedidos$vl_venda_pedido <- fato_pedidos$Total_Pedido 
...
fato_pedidos$Total_Pedido <- gsub("[^0-9\\.\\,]", "", fato_pedidos$Total_Pedido) # remove caracteres não numéricos
fato_pedidos$Total_Pedido <- gsub(",", ".", fato_pedidos$Total_Pedido) # substitui vírgulas por pontos
fato_pedidos$Total_Pedido <- as.numeric(fato_pedidos$Total_Pedido) # converte para numérico
...
fato_pedidos %>%
  group_by(id_cidade) %>%
  summarise(`Valor médio pedido` = mean(Total_Pedido, na.rm = T)) %>%
  kable() %>%
  kable_styling(bootstrap_options = "striped",
                full_width = F,
                font_size = 12)

#Exploração visual do valor médio pedido
fato_pedidos %>%
  group_by(id_cidade) %>%
  mutate(pedido_medio = mean(Total_Pedido, na.rm = TRUE)) %>% 
  ggplot() +
  geom_point(aes(x = id_cidade, y = Total_Pedido),color = "orange", alpha = 0.5, size = 4) +
  geom_line(aes(x = id_cidade, y = pedido_medio, 
                group = 1, color = "Valor médio pedido por cidade"), size = 1.5) +
  scale_colour_viridis_d() +
  labs(x = "Cidade",
       y = "Valor médio pedido por cidade") +
...
ggplotly(
  ggplot(fato_pedidos, aes(x = "", y = Total_Pedido)) +
...
#variável dependente (pedido médio), com histograma
ggplotly(
  ggplot(fato_pedidos, aes(x = Total_Pedido)) +
    geom_density(aes(x = Total_Pedido), 
...
#Boxplot da variável dependente (Total Pedido) por cidade
ggplotly(
  ggplot(fato_pedidos, aes(x = id_cidade,y = Total_Pedido)) +
    geom_boxplot(aes(fill = id_cidade, alpha = 0.7)) +
    geom_jitter(width = 0.1, alpha = 0.5, size = 1.3, color = "darkorchid") +
    scale_fill_viridis_d() +
    labs(y = "Total_Pedido") +
    theme_classic() +
    ggtitle("Boxplots da variável 'Valor do Pedido' para as cidades")
)

#Boxplot alternativo da variável dependente (Total Pedido) por cidade
#pacote 'gghalves'
fato_pedidos %>%
  ggplot(aes(id_cidade, Total_Pedido, fill = id_cidade)) +
  geom_half_boxplot(
    outlier.colour = "red"
  ) +
  geom_half_dotplot(
  ...
  scale_fill_viridis_d() +
  theme_tq() +
  labs(title = "Boxplots da variável 'Total Pedido' para as cidades")

#Distribuições da variável 'Total Pedido' para as cidades, com boxplots
#pacote 'ggdist'
fato_pedidos %>%
  ggplot(aes(x = id_cidade, y = Total_Pedido, fill = id_cidade)) +
  ggdist::stat_halfeye(
    adjust = 0.5,
    justification = -.2,
    .width = 0,
  ...
  theme_tq() +
  labs(
    title = "Distribuições da variável 'Valor Pedidos' para as cidades",
...
ggplot(fato_pedidos, aes(x = Total_Pedido, y = id_cidade, fill = ..x..)) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
  scale_fill_viridis(name = "Total_Pedido", option = "turbo", direction = -1) +
  labs(
...
#Kernel density estimation (KDE) - função densidade de probabilidade da
#variável dependente (Total Pedido) por cidade
ggplotly(
  ggplot(fato_pedidos, aes(x = Total_Pedido)) +
...
fato_pedidos %>% 
  group_by(id_cidade) %>% 
  mutate(linhas = 1:n()) %>% 
  mutate(x = unlist(density(Total_Pedido, n = max(linhas))["x"]),
         y = unlist(density(Total_Pedido, n = max(linhas))["y"])) %>%
  ggplot() +
  geom_area(aes(x = x, y = y, group = id_cidade, fill = id_cidade), color = "black", alpha = 0.3) +
  geom_histogram(aes(x = Total_Pedido, y = ..density.., fill = id_cidade), 
...
ggplotly(
  fato_pedidos %>%
    ggplot(aes(x = sal_med, y = Total_Pedido)) +
...
ggplotly(
  fato_pedidos %>%
    ggplot(aes(x = idhm, y = Total_Pedido)) +
    geom_smooth(method = "lm", formula = y ~ x, se = F) +
    geom_point() +
    scale_colour_viridis_d() +
...
#Gráfico de Total Pedido x sal_med (OLS) por cidade separadamente
#(funções transition_states e animate do pacote gganimate)
#ggplot(fato_pedidos, aes(x=sal_med, y=Total_Pedido, color=id_cidade)) +
#  geom_point() +
#  geom_smooth(method = "lm", formula = y ~ x, se = F) +
#  transition_states(id_cidade, transition_length = 1, state_length = 2) + 
#  enter_fade() + 
#  exit_shrink() +
#  ease_aes('linear') + 
#  labs(x = "Salário Médio Mensal cidade onde foi feito o pedido",
#       y = "Total Pedido") +
#  scale_color_viridis_d() +
#  ggtitle("Total Pedido por cidade", subtitle = "Cidade: {closest_state}") +
#  theme_minimal() -> p
#
...
ggplotly(
  fato_pedidos %>%
    ggplot(aes(x = sal_med, y = Total_Pedido, color = id_cidade)) +
    geom_smooth(method = "lm", formula = y ~ x, se = F) +
    geom_point() +
    guides(color = "none") +
    scale_colour_viridis_d() +
    labs(x = "Salário Médio Mensal da cidade onde foi feito o pedido",
         y = "Valores dos Pedidos") +
    theme_bw()
)
...
base_exemplo <- fato_pedidos %>%
  filter(id_cidade %in% c("SJC","CPV","TTE","TMB")) %>%
  mutate(id_cidade = as.numeric(id_cidade))

scatter3d(Total_Pedido ~ sal_med + id_cidade, #função scatter3d do pacote car
          data = base_exemplo,
          fit = "linear")
...
modelo_nulo_hlm2 <- lme(fixed = Total_Pedido ~ 1, 
                        random = ~ 1 | idhm,
                        data = fato_pedidos,
...
modelo_ols_nulo <- lm(formula = Total_Pedido ~ 1, 
                      data = fato_pedidos)
...
modelo_intercept_hlm2 <- lme(fixed = Total_Pedido ~ sal_med,
                             random = ~ 1 | id_cidade,
                             data = fato_pedidos,
                             method = "REML")
...
modelo_intercept_inclin_hlm2 <- lme(fixed = Total_Pedido ~ sal_med,
                                    random = ~ sal_med | id_cidade,
                                    data = fato_pedidos,
                                    method = "REML")
...
modelo_final_hlm2 <- lme(fixed = Total_Pedido ~ sal_med + idhm + sal_med:idhm,
                         random = ~ sal_med | id_cidade,
                         data = fato_pedidos,
                         method = "REML")
